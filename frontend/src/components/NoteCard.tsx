...
export const NoteCard: React.FC<NoteCardProps> = React.memo(({ note, category, onClick }) => {
  const theme = category ? THEMES[category.theme_id] : THEMES['orange'];
  ...
  return (
    ...
    >
      <div className="flex justify-between items-baseline mb-2 opacity-90 text-sm font-bold tracking-wide text-[#4A3B2C]">
        ...
        <span className="truncate max-w-[50%]">{category?.name || note.category_name}</span>
      </div>
      
      <h3 className="text-2xl font-bold font-serif mb-1 line-clamp-2 leading-tight text-[#4A3B2C]">
