...
interface CategoryManagerProps {
  categories: Category[];
  notes: Note[];
  onCreateCategory: (data: { name: string; theme_id: string }) => Promise<void>;
  onUpdateCategory: (id: number, data: { name?: string; theme_id?: string }) => Promise<void>;
  onDeleteCategory: (id: number, action: 'delete_all' | 'move', targetId?: number) => Promise<void>;
  onBulkMoveNotes: (noteIds: number[], targetCategoryId: number) => Promise<void>;
  onClose: () => void;
}
...
export const CategoryManager: React.FC<CategoryManagerProps> = ({
  ...
  const saveEditing = async (id: number) => {
    ...
    await onUpdateCategory(id, { name: tempName });
    setEditingId(null);
  };

  const changeColor = async (id: number, themeId: string) => {
    await onUpdateCategory(id, { theme_id: themeId });
  };

  const handleDelete = async (id: number, action: 'delete_all' | 'move', moveTargetId?: number) => {
    await onDeleteCategory(id, action, moveTargetId);
    setDeleteModeId(null);
  };
  ...
  const handleMoveNotes = async () => {
    ...
    await onBulkMoveNotes(Array.from(selectedNoteIds), targetCatId);
    setMoveModeId(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#4A3B2C]/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#FBF7F0] w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-[#8B7355]/20 scale-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 border-b border-[#8B7355]/10 flex justify-between items-center bg-white/50">
          <h2 className="text-2xl font-bold font-serif text-[#4A3B2C]">Manage Categories</h2>
          <button 
            onClick={onClose} 
              ...
              <div key={cat.id} className="bg-white p-4 rounded-xl shadow-sm border border-[#8B7355]/10 transition-all hover:border-[#8B7355]/30">
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  {editingId === cat.id ? (
                    <div className="flex-1 flex items-center gap-2">
                        <input 
                        autoFocus
                        className="flex-1 text-lg font-bold text-[#4A3B2C] bg-[#F3EFE6] px-3 py-1.5 rounded-lg focus:outline-none border-2 border-[#8B7355]/30 focus:border-[#8B7355] font-serif"
                        value={tempName}
                    ...
                    <span className="flex-1 text-lg font-bold text-[#4A3B2C] font-serif">{cat.name}</span>
                  )}
                  ...
                  <div className="flex gap-3 pl-8 pb-2 animate-in slide-in-from-top-1 fade-in duration-200">
                    {Object.entries(THEMES).map(([key, theme]) => (
                      <button
                        ...
                        title={theme.label}
                      >
                         {cat.theme_id === key && <Check size={14} className="text-[#4A3B2C]" strokeWidth={3} />}
                      </button>
                    ))}
                  </div>
                )}
                ...
                {deleteModeId === cat.id && (
                  <div className="mt-4 p-5 bg-red-50/50 rounded-xl border border-red-100 animate-in slide-in-from-top-2">
                    <h4 className="font-bold font-serif text-[#C0392B] mb-2 text-lg">Delete &quot;{cat.name}&quot;?</h4>
                    <p className="text-sm text-[#C0392B]/80 mb-4 font-medium">There are {notes.filter(n => n.category_id === cat.id).length} notes in this category.</p>
                      ...
                      {categories.length > 1 && (
                        <div className="flex gap-2">
                          <select 
                            ...
                            onChange={(e) => setTargetCatId(Number(e.target.value))}
                            defaultValue=""
                          >
                ...
                {moveModeId === cat.id && (
                  <div className="mt-4 p-5 bg-[#F3EFE6] rounded-xl border border-[#8B7355]/20 animate-in slide-in-from-top-2">
                    <h4 className="font-bold font-serif text-[#4A3B2C] mb-3 text-lg">Move Notes from &quot;{cat.name}&quot;</h4>
                    
                    <div className="mb-4 max-h-48 overflow-y-auto border border-[#8B7355]/10 rounded-lg bg-white p-2 shadow-inner">
                      {notes.filter(n => n.category_id === cat.id).length === 0 ? (
                         <p className="text-sm text-[#8B7355]/50 italic p-2">No notes to move.</p>
                      ) : (
                        notes.filter(n => n.category_id === cat.id).map(note => (
