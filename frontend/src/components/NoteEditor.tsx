...
export const NoteEditor: React.FC<NoteEditorProps> = ({ 
  ...
  // Save Function
  const saveChanges = useCallback((newTitle: string, newContent: string, newCatId: number | null) => {
  ...
  const currentTheme = THEMES[activeCategory?.theme_id] || THEMES['orange'];
      ...
      >
                    ...
                    {categories.map((cat) => (
                      ...
                      >
                         <div 
                           ...
                           style={{ backgroundColor: THEMES[cat.theme_id]?.bg }} 
                         />
                         {cat.name}
                      </button>
                    ))}
        ...
        <div className="flex-1 flex flex-col px-6 md:px-8 pb-8 overflow-y-auto">
          <input
            ...
            className="bg-transparent text-4xl sm:text-5xl font-bold font-serif text-[#4A3B2C] placeholder-[#4A3B2C]/30 focus:outline-none mb-6 w-full leading-tight"
          />
        ...
        {showDeleteConfirm && (
          <div className="absolute inset-0 bg-black/10 backdrop-blur-[2px] z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
             <div className="bg-[#FBF7F0] p-6 rounded-2xl shadow-xl max-w-sm w-full border border-[#8B7355]/10 transform scale-100 animate-in zoom-in-95 duration-200">
                <h3 className="text-xl font-bold font-serif text-[#4A3B2C] mb-2">Delete Note?</h3>
